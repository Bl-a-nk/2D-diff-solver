...
%% 5 point difference discretization
Ac=zeros(n,n); Al=zeros(n,n); Ar=zeros(n,n); At=zeros(n,n); Ab=zeros(n,n);
for i=1:n-1
    for j=1:n-1
...
while tol>tolerance
    for i=1:n^2
        phi(i)=(1-w)*phi_laststep(i)+(w/A(i,i))*(s(i)-A(i,[1:i-1])*phi([1:i-1])-A(i,[i+1:n^2])*phi_laststep([i+1:n^2]));
    end
    tol=abs(max(phi-phi_laststep));
    phi_laststep=phi;
end
